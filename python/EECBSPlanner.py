...
class CTNode:
    def __init__(self, num_agents):
        ...
        self.lb = 0  # Lower bound for EES
        self.h = 0   # Heuristic estimate for EES
...
class EECBSPlanner:
    ...
    def get_neighbors(self, state):
        ...
        if self.state_valid(forward, state) and self.transition_valid(state, forward):
        ...
        if self.state_valid(turn_left, state):
        ...
        if self.state_valid(turn_right, state):
    ...
    def update_one_step_errors(self, parent, children):
        min_cost = float('inf')
        ...
        if parent.h != 0:
            epsilon_d = sum([child.cost - min_cost for child in children]) / len(children)
            self.epsilon_d = 0.9 * self.epsilon_d + 0.1 * epsilon_d
            self.epsilon_h = 0.9 * self.epsilon_h + 0.1 * (min_cost - parent.cost)

    def plan(self, time_limit):
        open_list_cleanup = PriorityQueue()
        open_list_open = PriorityQueue()
        open_list_focal = PriorityQueue()
        close_list = set()

        initial_node = CTNode(self.env.num_of_agents)
        initial_cost = self.compute_solution(initial_node)
        initial_node.lb = initial_cost
        initial_node.h = 0

        open_list_cleanup.put((initial_node.lb, initial_node))
        open_list_open.put((initial_node.lb, initial_node))
        open_list_focal.put((initial_node.cost, initial_node))

        while not open_list_cleanup.empty():
            if not open_list_focal.empty():
                focal_best = open_list_focal.get()[1]
                if focal_best.cost <= self.w * open_list_cleanup.queue[0][0]:
                    current_node = focal_best
                else:
                    current_node = open_list_cleanup.get()[1]
            else:
                current_node = open_list_cleanup.get()[1]
            ...
            conflict = self.get_first_conflict(current_node.solutions_dict)
            if conflict.conflict_type == -1:
                actions = self.compute_actions(current_node.solutions_dict)
                return actions, current_node.solutions_dict
            ...
            for agent_id in constraints_dict.keys():
                ...
                new_node.lb = new_cost
                new_node.h = current_node.h + self.epsilon_h

                if new_node not in close_list:
                    open_list_cleanup.put((new_node.lb, new_node))
                    open_list_open.put((new_node.lb + new_node.h, new_node))
                    open_list_focal.put((new_node.cost, new_node))
                    children.append(new_node)

            self.update_one_step_errors(current_node, children)
